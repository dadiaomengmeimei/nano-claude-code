...
> **~2,300 行 TypeScript。19 个源文件。4 个运行时依赖。**
...
- 🛠️ **7 个核心工具** — Bash、FileRead、FileEdit、FileWrite、Grep、Glob、SubAgent
- 🔐 **权限系统** — 只读工具自动放行；写入/Shell 操作需要用户确认
- 🗜️ **对话压缩** — `/compact` 命令摘要对话历史，节省 token（对标 Claude Code）
- 🤖 **子代理** — 将复杂子任务委派给独立的 SubAgent（对标 Claude Code 的 Task 工具）
- ✅ **测试覆盖** — 41 个测试用例，覆盖 agent loop、工具、schema、compact
...
![nano-claude-code 运行效果](imgs/image.png)
